...
/****************************************************************************
* Class cPerfiles: Clase de Perfiles de Usuarios de Backoffice                *
****************************************************************************/

class clsBackOfficePerfiles {
	...
	function chequearPerfil($strPerfil, $strPerfilAnterior, $strDescripcion, $blnNuevoPerfil = false){
		...
		$this->errorPerfil = (isset($objCheck->arrErrors["strPerfil"])) ? $objCheck->arrErrors["strPerfil"] : "";
	...
	function insertPerfil($strPerfil, $strDescripcion, $arrSecciones){

		$this->chequearPerfil($strPerfil, "", $strDescripcion, true);
		...
		if ($this->intPerfil){
			...
			$this->updateSecciones($this->intPerfil, $arrSecciones);
		}
	...
	function updatePerfil($intPerfil, $strPerfil, $strPerfilAnterior, $strDescripcion, $arrSecciones){

		$this->chequearPerfil($strPerfil, $strPerfilAnterior, $strDescripcion, false);
		...
		$blnUpdatedOk = mysql_affected_rows();
		var_dump($blnUpdatedOk);
		if ($blnUpdatedOk){
			/* Grabo las secciones disponibles para el perfil */
			$this->updateSecciones($intPerfil, $arrSecciones);
		}
	...
	function updateSecciones($intPerfil, $arrSecciones){
		...
		if ($arrSecciones){
			for ($i = 0; $i < sizeOf($arrSecciones); $i++){
				if ($arrSecciones[$i][1]){
					...
					$strSQL .= "		VALUES ";
					$strSQL .= "			($intPerfil, " . $arrSecciones[$i][0] . ", " . $arrSecciones[$i][1] . ") ";
	...
	function getSecciones($intPerfil, $blnOrder = false){
		...
		if ($blnOrder)
			$strSQL .= "	ORDER BY s.DES_SECCION ";
		else
			$strSQL .= "	ORDER BY s.COD_SECCION ";
	...
	function getPermisoSeccion($intAcceso, $intPermiso){
		$arrCodigosCifras = array(
			"1" => 4,
			"2" => 3,
			"4" => 2,
			"8" => 1,
		...
		$strNumeroBinario = substr("0000" . decbin($intAcceso), -5);
		//echo ($strNumeroBinario . " - " . $intPermiso . " :: " . $strNumeroBinario[$arrCodigosCifras[$intPermiso]] . "<br>");
		return ($strNumeroBinario[$arrCodigosCifras[$intPermiso]] == 1);
	}
