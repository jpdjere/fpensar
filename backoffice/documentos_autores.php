...
$objTemplate->set_file(array(
	...
	"PAGINA" => TEMPLATES_DIR . "documentos_autores.html",
...
$objTemplate->set_block("NOTAS", "MODIFICAR_NOTAS_UPDATE", "modificar_documentos_autores_update");
$objTemplate->set_block("NOTAS", "MODIFICAR_NOTAS_DELETE", "modificar_documentos_autores_delete");
$objTemplate->set_block("NOTAS", "NOTAS_AGREGAR_HOME", "documentos_autores_agregar_home");
$objTemplate->set_block("NOTAS", "NOTAS_BORRAR_HOME", "documentos_autores_borrar_home");
$objTemplate->set_block("PAGINA", "MODIFICAR_NOTAS_ADD", "modificar_documentos_autores_add");
$objTemplate->set_block("PAGINA", "NOTAS_VACIO_BUSQUEDA", "documentos_autores_vacio_busqueda");
$objTemplate->set_block("PAGINA", "NOTAS_VACIO", "documentos_autores_vacio");
...
if ($intCantidadRegistros){
	...
	for ($i = 0; $i < $objNotas->intTotal; $i++){
		...
		if ($blnPermisoModificacion)
			$objTemplate->parse("modificar_documentos_autores_update", "MODIFICAR_NOTAS_UPDATE");
		else
			$objTemplate->set_var("modificar_documentos_autores_update", "");

		if ($blnPermisoBaja)
			$objTemplate->parse("modificar_documentos_autores_delete", "MODIFICAR_NOTAS_DELETE");
		else
			$objTemplate->set_var("modificar_documentos_autores_delete", "");
	...
	if ($strBusqueda){
		...
		$objTemplate->set_var("documentos_autores_vacio", "");
		$objTemplate->parse("documentos_autores_vacio_busqueda", "NOTAS_VACIO_BUSQUEDA");
	}else{
		$objTemplate->set_var("notas", "");
		$objTemplate->set_var("documentos_autores_vacio_busqueda", "");
		$objTemplate->parse("documentos_autores_vacio", "NOTAS_VACIO", true);
	}
}

if ($blnPermisoAlta)
	$objTemplate->parse("modificar_documentos_autores_add", "MODIFICAR_NOTAS_ADD");
else
	$objTemplate->set_var("modificar_documentos_autores_add", "");
